...
def split_string_by_limit(input_string: str, char_limit: int) -> list[str]:
    ...
    for word in words:
        # If the line is empty, start it with the first word.
        if not current_line:
            current_line = word
            continue

        # If adding the next word fits, add it.
        if len(current_line) + len(word) + 1 <= char_limit:
            current_line += " " + word
        # Otherwise, the current line is finished. Start a new one.
        else:
            lines.append(current_line)
            current_line = word

    # Add the last line to the results.
    lines.append(current_line)
