...
from src.webui.modules import shared, ui, utils
from src.webui.modules.evaluate import (
    calculate_perplexity,
    generate_markdown_table,
    save_past_evaluations
)
from src.webui.modules.logging_colors import logger
from src.webui.modules.models import reload_model
from src.webui.modules.utils import natural_keys
        ...
        with gr.Tab('Train LoRA', elem_id='lora-train-tab'):
            ...
            with gr.Row():
                with gr.Column():
                    ...
                    with gr.Accordion(label='Target src.webui.modules', open=False):
                        gr.Markdown("Selects which src.webui.modules to target in training. Targeting more src.webui.modules is closer to a full fine-tune at the cost of increased VRAM requirements and adapter size.\nNOTE: Only works for model_id='llama', other types will retain default training behavior and not use these settings.")
                        with gr.Row():
                            with gr.Column():
...
def do_train(lora_name: str, always_override: bool, q_proj_en: bool, v_proj_en: bool, k_proj_en: bool, o_proj_en: bool, gate_proj_en: bool, down_proj_en: bool, up_proj_en: bool, save_steps: int, micro_batch_size: int, batch_size: int, epochs: int, learning_rate: str, lr_scheduler_type: str, lora_rank: int, lora_alpha: int, lora_dropout: float, cutoff_len: int, dataset: str, eval_dataset: str, format: str, eval_steps: int, raw_text_file: str, overlap_len: int, newline_favor_len: int, higher_rank_limit: bool, warmup_steps: int, optimizer: str, hard_cut_string: str, train_only_after: str, stop_at_loss: float, add_eos_token: bool, min_chars: int, report_to: str):
    ...
    # Populate target_modules list with chosen X_proj src.webui.modules. Llama-based models only atm, non-llama will revert to default behavior.
