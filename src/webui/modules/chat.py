...
from src.webui.modules import utils
from src.webui.modules.extensions import apply_extensions
from src.webui.modules.html_generator import (
    chat_html_wrapper,
    convert_to_markdown,
    make_thumbnail
)
from src.webui.modules.logging_colors import logger
from src.webui.modules.text_generation import (
    generate_reply,
    get_encoded_length,
    get_max_prompt_length
)
from src.webui.modules.utils import delete_file, get_available_characters, save_file
...
def chatbot_wrapper(text, state, regenerate=False, _continue=False, loading_message=True, for_ui=False, bot=None):
    ...
    # Generate
    reply = None
    for j, reply in enumerate(generate_reply(prompt, state, stopping_strings=stopping_strings, is_chat=True, for_ui=for_ui, bot=bot)):
...
def generate_chat_reply(text, state, regenerate=False, _continue=False, loading_message=True, for_ui=False, bot=None):
    history = state['history']
...
def generate_chat_reply_wrapper(text, state, regenerate=False, _continue=False, bot=None):
    '''
    ...
    history = state['history']
    for i, history in enumerate(generate_chat_reply(text, state, regenerate, _continue, loading_message=True, for_ui=True, bot=None)):
